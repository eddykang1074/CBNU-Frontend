function getRendomElement<T>(list: T[]): T {
...
function getRendomString(list: string[]): string {
  //전달된 배열목록에서 랜덤하게 배열 단일아이템을 반환한다.
  const randIndx = Math.floor(Math.random() * list.length);
  return list[randIndx];
}

function getRendomNumber(list: number[]): number {
  //전달된 배열목록에서 랜덤하게 배열 단일아이템을 반환한다.
  const randIndx = Math.floor(Math.random() * list.length);
  return list[randIndx];
}
...
const randomString = getRendomString(["A", "B", "C"]);
console.log("문자열 배열에서 랜덤한 문자 추출하기:", randomString);

//특정타입에 최적화된 함수를 별도로 만들고 사용하기
const randomNumber = getRendomNumber([1, 2, 3]);
...
const randomString1 = getRendomElement(["A", "B", "C"]);
const randomNumber1 = getRendomElement([1, 2, 3]);

const randomUser1 = getRendomElement([
